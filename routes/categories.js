...
router.get("/:id", async (req, res) => {
  try {
    ...
    // Tìm danh mục theo ID từ request
    const category = await Category.findById(req.params.id);
    ...
    // Tạo danh mục phân cấp bắt đầu từ ID của category đã tìm
    const categoryData = createCat(categoryList, category._id);

    return res.status(200).json({
      categoryData, // Trả về danh mục với cấu trúc phân cấp
    });
